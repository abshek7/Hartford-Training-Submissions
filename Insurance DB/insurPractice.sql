...
SELECT ClaimStatus, SUM(ClaimAmount) AS TotalClaimAmount
FROM Claims
...
SELECT FirstName, YEAR(DateOfBirth) AS BirthYear
FROM Customers;

SELECT AssignmentID, MONTH(StartDate) AS StartMonth
FROM PolicyAssignments;
...
SELECT AssignmentID,
       DATEDIFF(YEAR, StartDate, EndDate) AS PolicyDurationYears
FROM PolicyAssignments;
...
--Customers who have NOT taken any policy
SELECT *
FROM Customers
...
--1. Categorize policies based on premium amount (CASE)
SELECT PolicyName, PremiumAmount,
       CASE
           WHEN PremiumAmount < 10000 THEN 'Low Premium'
           WHEN PremiumAmount BETWEEN 10000 AND 15000 THEN 'Medium Premium'
           ELSE 'High Premium'
...
SELECT 
    CASE
        WHEN PremiumAmount < 10000 THEN 'Low'
        WHEN PremiumAmount BETWEEN 10000 AND 15000 THEN 'Medium'
        ELSE 'High'
    END AS PremiumCategory,
...
SELECT 
    ...
    GROUPING(ClaimStatus) AS IsSubtotal
FROM Claims
GROUP BY ROLLUP (ClaimStatus);
